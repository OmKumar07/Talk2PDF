...
embedder = SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2")  # Match the ingest model
...
def generate_question_variants(question, intent):
    """
    Generate focused question variants for better retrieval (optimized for performance)
    """
    variants = [question]  # Always include original
    
    # Add only ONE most relevant variant based on intent
    if intent['requires_definition']:
        term = extract_main_term(question)
        if term:
            variants.append(f"definition of {term}")
    elif intent['requires_summary']:
        variants.append("main points key concepts")
    elif intent['requires_process']:
        variants.append("steps procedure method")
    elif intent['keywords'] and len(intent['keywords']) > 0:
        keyword_query = " ".join(intent['keywords'][:2])  # Top 2 keywords only
        variants.append(keyword_query)
    
    return variants[:2]  # Limit to 2 variants max for performance
...
def answer_query(doc_id, question, top_k=8, threshold=0.3):
    ...
    """
    try:
        # Step 0: Validate input and file existence
        if not doc_id or not question:
            return {
                "answer": "Please provide both a document ID and a question.",
                "sources": [],
                "score": 0.0
            }
        
        import os
        index_path = f"storage/{doc_id}.index"
        chunks_path = f"storage/{doc_id}_chunks.json"
        
        if not os.path.exists(index_path):
            return {
                "answer": f"Document index not found for ID: {doc_id}. Please upload the document first.",
                "sources": [],
                "score": 0.0
            }
        ...
        if not os.path.exists(chunks_path):
            return {
                "answer": f"Document chunks not found for ID: {doc_id}. Please re-upload the document.",
                "sources": [],
                "score": 0.0
            }
        ...
        print(f"Error in answer_query: {type(e).__name__}: {str(e)}")
        import traceback
        traceback.print_exc()
        
        error_message = str(e) if str(e) else f"Unknown {type(e).__name__} error"
        return {
            "answer": f"I encountered an error while processing your question: {error_message}. Please try rephrasing your question or check if the document was uploaded correctly.",
            "sources": [],
            "score": 0.0
        }
